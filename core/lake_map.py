...
(data/lake_spots.csv, core/lake_spots.py). Both are always drawn - no
layer-toggle checkboxes - since there's nothing here worth hiding.
...
the real digitized shoreline outline, all behind a folium.LayerControl,
plus an explanatory dialog on the page about where that data came from.
Per user feedback, all of that was removed in favor of a simpler map: the
one thing that's unambiguously real, GPS-placed data (fish attractors)
plus the spots the angler records themselves - no modeled/derived layers,
no checkboxes to manage, no disclaimer needed. Support for clicking
anywhere on the lake (not just on a marker) to drop a new pin is handled
by streamlit-folium in the page that renders this map
(pages/2_Lake_Map.py).
"""
