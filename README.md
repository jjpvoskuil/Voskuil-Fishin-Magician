...
- **"🎣 Fishing Activity"** on the Home page (punch-list #91) - sits right under "Today at a
  ...
  **📅 Today** and **🗓️ This Week** (Sunday-Saturday, lake-local - added as a same-punch-list
  follow-up so both periods offer identical categories), each showing: a combined roster of every
  angler who's either got a Spot Session open right now anywhere on the lake (🟢), or has logged at
  least one row in that period (⚪) even if that session's already ended - "currently logged in
  and/or posted a session," per the angler's own framing - sorted active anglers first, then by
  fish caught in that period (both the roster order and the "currently logged in" definition were
  ...
  one final "All anglers - total" row at the bottom. A group-logged small-fish entry's weight is
  per-fish, not the whole group's combined weight, so every total here multiplies weight by count
  ...
  wrappers scoping it to one day or a Sunday-start week, so the exact same award/table functions
  render either tab - kept deliberately separate from `pages/8_Leaderboard.py`'s own all-time
  ranking logic to avoid any regression risk on that already-shipped page.
